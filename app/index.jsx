...
import { loadAdminData, setAdminSession, saveCurrentUser } from "./utils/storage";
  ...
  const onLogin = async () => {
    ...
    try {
      ...
      if (admin && username === admin.username && password === admin.password) {
        ...
        // Save current user as admin (id 'admin')
        await saveCurrentUser({ id: "admin", username: admin.username });
    ...
    // Regular user: record current user info locally (id = username + timestamp) and clear admin session
    await setAdminSession(false);
    const userId = `${username}_${Date.now()}`;
    await saveCurrentUser({ id: userId, username });
    router.push("/home");
  };
