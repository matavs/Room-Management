  ...
  return (
    <TouchableOpacity activeOpacity={0.95} onPress={() => onPress?.(item)} style={[styles.card, status === "Occupied" && styles.cardActive]}>
      <View style={styles.left}>
        <View style={[styles.avatar, { backgroundColor: item.color || "#4C6EF5" }]}>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.eventBox}>
          <View style={{ flexDirection: "row", alignItems: "center", flexWrap: "wrap" }}>
            ...
            {upcoming && !activeBooking && <Text style={styles.gap}>Next: {formatTime(upcoming.startTime)}</Text>}
          </View>

          <Text numberOfLines={2} style={styles.eventText}>
            {displayTitle}
            {(displayStart && displayEnd) ? <Text style={styles.timeRange}> • {new Date(displayStart).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})} - {new Date(displayEnd).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</Text> : null}
          </Text>
...
const styles = StyleSheet.create({
  ...
  cardActive: { borderColor: "rgba(244,67,54,0.4)", borderWidth: 1, backgroundColor: "#1E1515" },
  left: { marginRight: 12 },
  ...
  useBtn: { backgroundColor: "#2ECC71", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6, marginLeft: 8 },
  useBtnText: { color: "#000", fontWeight: "800", fontSize: 10 },
  leaveBtn: { backgroundColor: "#E74C3C", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6, marginLeft: 8 },
  ...
  gap: { marginLeft: 8, backgroundColor: "#34495E", color: "#fff", paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, fontSize: 10, marginTop: 4 },
